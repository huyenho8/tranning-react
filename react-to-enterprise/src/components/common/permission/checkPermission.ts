 ...
 *
 * For one-of we only need to find one record, so .some is sufficient * For all-of we want to match all roles, so we use .every
 */
...
export const checkPermission = (
   ...
   * entityOwnerId is only needed when checking if a user * is an owner of an entity such as comment, post, etc */
   ...
   * We need to bind the 'roles' array to make sure these functions are * run in the context of the array prototype.
   */
